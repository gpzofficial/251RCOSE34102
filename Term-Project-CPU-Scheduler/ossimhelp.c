...
int PrintHelp(const char *input)
{
	...
	if(input == NULL)
	{
		...
		printf("\tsave\t\tSave the current sessions as a file.\n");
		printf("\tload\t\tLoad the saved session to analyze the session. The opened session will NOT be saved.\n");
		printf("\tanalyze\t\tAnalyze the session by testing all of the algorithm, and compare the output.\n");
